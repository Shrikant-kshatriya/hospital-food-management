...
import BoxDetailsModal from "./BoxDetail";
...
const ManageMealBox = () => {
  ...
  const fetchBoxes = async () => {
    try {
      const res = await axios.get(`/deliveryPersonnel/${user._id}`);
      const response = await axios.get(`/orderBox?deliveryBy=${res.data._id}`);
  ...
  const handleUpdateBox = async (boxId, updatedData) => {
    try {
      await axios.patch(`/orderBox/${boxId}`, updatedData);
      toast.success("Box updated successfully.");
      fetchBoxes();
    } catch (error) {
      console.error("Error updating box:", error);
      toast.error("Failed to update the box.");
    }
  };
  ...
  return (
    <div className="p-4 space-y-6">
      <ToastContainer />
      <h2 className="text-2xl font-semibold">Manage Meal Boxes</h2>
      {boxes.length === 0 ? (
            ...
            <tbody className="text-center">
              {boxes.map((box, index) => (
                <tr key={box._id} className="hover:bg-indigo-50 border-b-4 border-b-white">
                  ...
                  <td className="px-4 py-2">{box.status} {box.deliveryTime && <span><br />{new Date(box.deliveryTime).toLocaleTimeString()}</span>}</td>
